...
namespace coen79_lab6
{
    ...
    private:
        //head and tail of linked list, if no items, both NULL
    	node *head_ptr;
    	node *tail_ptr;
        //points to the node with the current item (or NULL if no current)
    	node *cursor;
        //points to the node before current item (or NULL if no current/if cursor is at head)
    	node *precursor;
