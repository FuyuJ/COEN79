...
*/
// FILE: node.cpp
// CLASS implemented: node (see node.h for documentation)
#ifndef ITEM_CPP
#define ITEM_CPP
...
namespace coen79_lab7
{
    ...
    void list_tail_insert(node*& tail, const std::string &newName, const float &newPrice) {
        ...
        //set the link of tail node to the new node
        tail -> setLink(insert_ptr);
        //update tail pointer to new node since it is the end of the list now
        tail = tail -> getLink();
    ...
    void list_clear(node*& head) {
        //until the list is empty, keep removing the head node.
        while (head != NULL) {
    ...
    void list_head_remove(node*& head) {
        //create a new pointer also pointing to head node
        node *remove_ptr = head;
        //shift head node forward one
        head = head -> getLink();
        //delete old head
        delete remove_ptr;
