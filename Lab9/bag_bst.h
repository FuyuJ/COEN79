...
* Date: 11/29/2021
* Class: COEN 79L
* Title: Lab 9 - The Bag Class with a Binary Search Tree
*/
...
#pragma mark - Member Function Implementation
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
                ...
                //if not NULL, keep moving down tree
                if (cursor -> left() != NULL) {
                    cursor = cursor -> left();
                //else insert entry as child of cursor node
                } else {
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        while (cursor != NULL)
        {
            ...
            {
                //increment answer if target is found
    ...
    {
        if (root_ptr == addend.root_ptr) //self-addition b+=b
        { 
            bag<Item> copy = addend;
            insert_all(copy.root_ptr);
        }
        else //two different trees a+=b
    ...
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2)
    {
        //create new bag, add both arguments to new_bag 
        bag<Item> new_bag;
        new_bag += b1;
        new_bag += b2;
        return new_bag;
